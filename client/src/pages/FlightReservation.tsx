...
import { CompleteFlightTranscation, FlightDetail, FlightTransactionPayload } from "@myTypes/flight.types"
    ...
    const addToCart = async (price: number) => {
        try {
            ...
            const flightTransactions:FlightTransactionPayload[] = reservations.map(rsvp => {
                ...
                return generateFlightTransaction(flightDetail, rsvp.traveler, rsvp?.seat, 10);
            });
    
            const completeTransaction: CompleteFlightTranscation = {
                userId: user!.ID!,
                price: price,
